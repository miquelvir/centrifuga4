...
class TestPasswordResetServiceTryDecodeToken(WithApp):  # todo refractor config out
    def setUp(self):
        super().setUp()
        self.sut = Container.password_reset_service()
...
class TestPasswordResetServiceUpdatePassword(WithDatabase):
    ...
    def test_raises_bad_request_when_weak_password_deprecated(
        ...
        with self.app.app_context():
            with self.assertRaises(BadRequest):
                self.sut.update_user_password(UserMother.simple(), new_password)

    def test_raises_bad_request_when_weak_password(self):
        # Arrange
        self.sut.is_strong_enough_password = Mock(return_value=False)
        sample_new_password = ""
        ...
        with self.app.app_context():
            with self.assertRaises(BadRequest):
                self.sut.update_user_password(UserMother.simple(), sample_new_password)
    ...
    def test_updates_student_password_hash_in_db(self):
        ...
        with self.app.app_context():
            ...
            # Act
            self.sut.update_user_password(sample_user, new_password)
...
class TestPasswordResetServiceGetUserByEmail(WithDatabase):
    ...
    def test_raises_key_error_if_no_user(self):
        ...
        with self.app.app_context():
            ...
            with self.assertRaises(KeyError):
                self.sut.get_user_from_email(sample_email)
    ...
    def test_returns_correct_user(self):
        ...
        with self.app.app_context():
            ...
            # Assert
            user = self.sut.get_user_from_email(sample_email)
