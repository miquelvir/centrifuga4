...
class TestWKHTMLToPDFConfig(unittest.TestCase):
    ...
    @patch("server.pdfs.wkhtmltopdf.os.access", return_value=True)
    def test_uses_path_from_shutil_when_env_is_missing(self, mock_access, mock_which, mock_configuration):
    ...
    @patch("server.pdfs.wkhtmltopdf.pdfkit.configuration")
    @patch("server.pdfs.wkhtmltopdf.os.access", return_value=True)
    def test_prefers_explicit_environment_path(self, mock_access, mock_configuration):
    ...
    @patch("server.pdfs.wkhtmltopdf.shutil.which", return_value="/usr/bin/wkhtmltopdf")
    @patch("server.pdfs.wkhtmltopdf.os.access", return_value=True)
    def test_ignores_missing_environment_path_and_falls_back(self, mock_access, mock_which, mock_configuration):
    ...
    def test_raises_when_no_executable_can_be_found(self):
        with patch.dict(os.environ, {}, clear=True):
            with patch("server.pdfs.wkhtmltopdf.os.path.exists", return_value=False):
                with patch("server.pdfs.wkhtmltopdf.os.access", return_value=False):
