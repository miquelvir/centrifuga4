...
OUTPUT_BASE = ROOT / "tmp" / "isolated_pdf_template_test"
...
def sample_enrolment_context():
    return {
        ...
        "guardians": [{"email": "parent@example.com", "phone": "+34 600 111 111"}],
        "schedules": {"c1": ["Mon 16:00-17:30", "Wed 16:00-17:30"], "c2": ["Fri 18:00-20:00"]},
    }
...
def main(skip_pdf: bool):
    ...
    for output_name, builder, context in tests:
        ...
        if pdf_bytes is None:
            ...
            continue
        pdf_path = output_folder / f"{output_name}.pdf"
        write_file(pdf_path, pdf_bytes, mode="wb")
        print(f"Rendered PDF: {pdf_path}")

    print(f"Outputs written to: {output_folder}")
    print("Done.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Isolated native PDF builder tester")
    parser.add_argument("--skip-pdf", action="store_true", help="Skip PDF generation")
    args = parser.parse_args()
    if args.skip_pdf:
        print("skip-pdf is not supported for native PDF test script.")
    main(skip_pdf=args.skip_pdf)
