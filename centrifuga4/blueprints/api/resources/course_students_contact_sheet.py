...
# todo maybe not flatten exports
    ...
    def post(self, id_):
        ...
        with proxy as fr:
            ...
            for student in course.students:
                ...
                if student.email:
                    emails[student.enrollment_status]["student_emails"].add(
                        student.email
                    )

                for email in student.official_notification_emails:
                    emails[student.enrollment_status]["official_contact_emails"].add(
                        email
                    )
                ...
                for guardian in student.guardians:
                    if guardian.email:
                        emails[student.enrollment_status]["guardian_emails"].add(
                            guardian.email
                        )
            ...
            for status in emails:
                ...
                emails["all"]["guardian_emails"] = emails["all"][
                    "guardian_emails"
                ].union(emails[status]["guardian_emails"])
            ...
            for status in emails:
                ...
                spamwriter.writerow(
                    ["the previous email list has all emails available"]
                )
                spamwriter.writerow([])
