...
class EasyResource(Resource, SwaggerView):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        assert issubclass(cls.schema, SQLAlchemySchema), \
            "instances of EasyResource must initialise field schema with a valid Marshmallow schema, found %s" % type(cls.schema).__name__
        ...
        assert issubclass(cls.model, Model), \
            "instances of EasyResource must initialise field model with a valid SQLAlchemy model, found %s" % type(cls.model).__name__
        ...
        try:
            for permission in cls.permissions:
                assert type(permission) == type(Permission), "instances of EasyResource must initialise field permissions " \
                                                       "with an iterable of Flask Principal permissions, " \
                                                       "found an iterable of '%s'" % type(permission)

        except TypeError:
            assert False, "instances of EasyResource must initialise field permissions with an iterable of Flask " \
                          "Principal permissions, found no iterable but '%s'" % type(cls.permissions)
