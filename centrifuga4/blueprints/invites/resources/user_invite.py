...
def generate_signup_link(_token, _email, frontend_url=None):
    return merge_url_query_params(
        "%s/signup"
        % (frontend_url if frontend_url else current_app.config["FRONTEND_SERVER_URL"]),
...
class UserInviteCollectionRes(Resource):
    ...
    def post(self):
        ...
        job = q.enqueue_call(
            func=my_job,
            args=(
                *generate_signup_link(token, user_email),
                user_email,
            ),
            result_ttl=5000,
        )
...
if __name__ == "__main__":
    ...
    clean_needs = [
        {"type": "res", "name": "students"},
        {"type": "action", "name": "get"},
    ]
    # simulate one
    token = jwt.encode(
        {"userEmail": user_email, "needs": clean_needs},
    ...
    job = q.enqueue_call(
        ...
        args=(
            *generate_signup_link(
                token, user_email, frontend_url="https://127.0.0.1:4999"
            ),
            user_email,
        ),
        result_ttl=5000,
