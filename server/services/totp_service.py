...
class TotpService:
    ...
    def encrypt_totp_secret(
        totp_secret, encryption_key: str = current_app.config["TOTP_ENCRYPTION_KEY"]
    ) -> str:
        return (
            Fernet(bytes(encryption_key, "utf-8"))
    ...
    def decrypt_totp_secret(
        encrypted_secret,
        encryption_key: str = current_app.config["TOTP_ENCRYPTION_KEY"],
    ) -> str:
        return (
            Fernet(bytes(encryption_key, "utf-8"))
