...
from server.emails.emails.pre_enrolment_email import my_job as pre_enrolment_email_job  # todo rename
from server.emails.emails.password_reset_email import my_job as password_reset_email_job  # todo rename
from server.emails.emails.password_change_email import my_job as password_changed_email_job  # todo rename


def when_student_enrolled(_, student: Student) -> None:
    thread = Thread(target=pre_enrolment_email_job, args=(student.official_notification_emails, student.id))
    thread.start()


def when_user_password_reset_request(_, user: User, token: str) -> None:
    def generate_password_reset_url(user: User, token: str, language: str = "eng",
                                    frontend_url: str = current_app.config["FRONTEND_SERVER_URL"]) -> str:
    ...
    thread = Thread(
        ...
        args=(generate_password_reset_url(user, token, "cat"),
